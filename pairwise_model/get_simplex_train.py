"""Get the simplex train. Written by Markus Ekvall 2018-07-09."""
import numpy as np
...
def get_simplex_train_gibbs(neuron, simplex, samples, simplex_train):
    """
    Simplex train used for the Gibss sampling.
...
def get_simplex_train2(samples):
    """
    Simplex train for 2 dimensional.
...
def get_simplex_train3(samples):
    """
    The simplex train for 3 dimensional.
    ...
    samples = sp.csc_matrix(samples)
    [n, m] = np.shape(samples)
    simplex_train = np.zeros((n, m/4))
    for simplex in range(0, m/4):
        first_spike_idx1 = samples[:, simplex*4] == 1
        intermediate_1 = first_spike_idx1.nonzero()[0] + 1
        intermediate_1 = intermediate_1[intermediate_1 < n]
        intermediate_spike_1 = samples[intermediate_1, 1+simplex*4] == 1

        intermediate_2 = [intermediate_spike_1.nonzero()[0]]
        intermediate_2 = intermediate_2[intermediate_2 < n]
        intermediate_spike_2 = samples[intermediate_2, 2+simplex*4] == 1
        sink_1 = intermediate_2[intermediate_spike_2.nonzero()[0]]
        sink_2 = intermediate_2[intermediate_spike_2.nonzero()[0]]+1
        sink_1 = sink_1[sink_1 < n]
        sink_2 = sink_2[sink_2 < n]
        sink_1_spike = samples[sink_1, 3+simplex*4] == 1
        sink_2_spike = samples[sink_2, 3+simplex*4] == 1
        iX1 = list(sink_1[sink_1_spike.nonzero()[0]]-1)
        iX2 = list(sink_2[sink_2_spike.nonzero()[0]]-2)
        first_spike_idx1 = samples[:, simplex*4] == 1
        intermediate_1 = first_spike_idx1.nonzero()[0] + 1
        intermediate_1 = intermediate_1[intermediate_1 < n]
        intermediate_spike_1 = samples[intermediate_1, 1+simplex*4] == 1
        intermediate_2 = [intermediate_spike_1.nonzero()[0]+1]
        intermediate_2 = intermediate_2[intermediate_2 < n]
        intermediate_spike_2 = samples[intermediate_2, 2+simplex*4] == 1
        sink_1 = intermediate_2[intermediate_spike_2.nonzero()[0]]
        sink_2 = intermediate_2[intermediate_spike_2.nonzero()[0]]+1
        ...
        sink_1_spike = samples[sink_1, 3+simplex*4] == 1
        sink_2_spike = samples[sink_2, 3+simplex*4] == 1
        iX4 = list(sink_1[sink_1_spike.nonzero()[0]]-2)
        iX5 = list(sink_2[sink_2_spike.nonzero()[0]]-3)
        iX3 = []
...
def get_simplex_train_4(samples):
    """
    Simplex train for 4 dimensional.
    ...
    samples = sp.csc_matrix(samples)
    [n, m] = np.shape(samples)
    simplex_train = np.zeros((n, m/5))

    for simplex in range(0, m/5):
        first_spike_idx1 = samples[:, simplex*5] == 1
        intermediate_1 = first_spike_idx1.nonzero()[0] + 1
        intermediate_1 = intermediate_1[intermediate_1 < n]
        intermediate_spike_1 = samples[intermediate_1, 1+simplex*5] == 1
        intermediate_2 = [intermediate_spike_1.nonzero()[0]]
        intermediate_2 = intermediate_2[intermediate_2 < n]
        intermediate_spike_2 = samples[intermediate_2, 2+simplex*5] == 1
        intermediate_3 = [intermediate_spike_2.nonzero()[0]]
        intermediate_3 = intermediate_3[intermediate_3 < n]
        intermediate_spike_3 = samples[intermediate_3, 3+simplex*5] == 1
        sink_1 = intermediate_3[intermediate_spike_3.nonzero()[0]]
        sink_2 = intermediate_3[intermediate_spike_3.nonzero()[0]]+1
        sink_1 = sink_1[sink_1 < n]
        sink_2 = sink_2[sink_2 < n]
        sink_1_spike = samples[sink_1, 4+simplex*5] == 1
        sink_2_spike = samples[sink_2, 4+simplex*5] == 1
...
def get_simplex_train_5(samples):
    """
    Simplex train for 5 dimensional.
    ...
    samples = sp.csc_matrix(samples)
    [n, m] = np.shape(samples)
    simplex_train = np.zeros((n, m/6))
    for simplex in range(0, m/6):
        # NB Markus
        first_spike_idx1 = samples[:, simplex*6] == 1
        intermediate_1 = first_spike_idx1.nonzero()[0] + 1
        intermediate_1 = intermediate_1[intermediate_1 < n]
        intermediate_spike_1 = samples[intermediate_1, 1+simplex*6] == 1

        intermediate_2 = [intermediate_spike_1.nonzero()[0]]
        intermediate_2 = intermediate_2[intermediate_2 < n]
        intermediate_spike_2 = samples[intermediate_2, 2+simplex*6] == 1

        intermediate_3 = [intermediate_spike_2.nonzero()[0]]
        intermediate_3 = intermediate_3[intermediate_3 < n]
        intermediate_spike_3 = samples[intermediate_3, 3 + simplex*6] == 1

        intermediate_4 = [intermediate_spike_3.nonzero()[0]]
        intermediate_4 = intermediate_4[intermediate_4 < n]
        intermediate_spike_4 = samples[intermediate_4, 4+simplex*6] == 1
        sink_1 = intermediate_spike_4[intermediate_spike_4.nonzero()[0]]
        sink_2 = intermediate_4[intermediate_spike_4.nonzero()[0]]+1
        sink_1 = sink_1[sink_1 < n]
        sink_2 = sink_2[sink_2 < n]
        sink_1_spike = samples[sink_1, 4+simplex*5] == 1
        sink_2_spike = samples[sink_2, 4+simplex*5] == 1
...
def set_row_csc(D, col_idx, new_col):
    ...
    assert col_idx < D.shape[1], \
        "The col index ({0}) shall be smaller than"\
        "the number of col in array ({1})" \
