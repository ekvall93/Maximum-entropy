...
def sample_pairwise(samples,J,n_steps):
    """
    ...
    #rand = np.array(genfromtxt('ZxZ.csv', delimiter=',')).astype(float)
    rand =np.random.uniform(low=0.0, high=1.0, size=(n,n_steps))#.reshape((N,1))
    for j in range(0,n_steps):
        delta_E = J[neuron_id, neuron_id].astype(float) + 2*samples.dot(J_offdiag[:,neuron_id]).astype(float)
        p_spike = 1./(1+np.exp(delta_E))
        N = np.size(p_spike)
        p_spike = p_spike.reshape((N,)).astype(float)
        #rand =np.random.uniform(low=0.0, high=1.0, size=(N,))#.reshape((N,1))
...
def set_row_csc(D, col_idx, new_col):
    ...
    assert col_idx < D.shape[1], \
            'The col index ({0}) shall be smaller than the number of col in array ({1})' \
    ...
    D.indices = np.r_[D.indices[:idx_start_col], np.arange(N_rows), D.indices[idx_end_col:]]
    D.indptr = np.r_[D.indptr[:col_idx + 1], D.indptr[(col_idx + 1):] + additional_nnz]
    return D
